...
- (?) при отключении скоринга блока слать remove событие, прятать скор индикатор
- (?) не сохранять на бэк (isCorrect === null) ответы, кроме последнего (чтобы восстановить состояние)
- (?) isAcceptableAnswer
- инит/дестрой в стратегиях, вызывать из BlockApi
- создавать api сразу, инициализировать как загрузятся нужные ответы
